...
void generate_fully_coherent_subcircuits_test(size_t subcircuit_vertices, size_t subcircuit_number/*, const std::string& test_file_name*/) {
    generate_test_file("R1C1L1.txt", "define R1C1L1(A, B)\n A -- B, 1R;1C;1L;");
    std::stringstream circuit_, answer_;
    circuit_ << "include R1C1L1.txt\n";
    for (size_t subcircuit_index = 0; subcircuit_index < subcircuit_number; subcircuit_index++) {
        ...
        for (size_t outcoming = subcircuit_vertex_first; outcoming <= subcircuit_vertex_last; outcoming++) {
            for (size_t incoming = outcoming + 1; incoming <= subcircuit_vertex_last; incoming++) {
                circuit_ << "R1C1L1(" << outcoming << ", " << incoming << ")\n";
                answer_ << outcoming << " -- " << incoming << ": 0 A, 0;\n";
            }
        }
    }
    std::stringstream inp(circuit_.str());
    input_preworker preworker(inp, "test", 0);
    preworker.perform_prework();
    circuit<std::complex<double>> test_circuit(preworker.get_output());
    test_circuit.calculate_edge_current();
    std::string answer = answer_.str();
    answer.pop_back();
    EXPECT_EQ(answer, test_circuit.get_edge_current_answer());
//    std::ofstream test_file("..\\t\\e2e_three_phase_test\\" + test_file_name + "_test.txt");
//    std::ofstream answer_file("..\\t\\e2e_three_phase_test\\" + test_file_name + "_answer.txt");
...
void generate_consistent_circuit_test(size_t vertex_last, CircuitType type/*, const std::string& test_file_name*/) {
    generate_test_file("R1.txt", "define R1(A,B)\n A -- B, 1R;0C;0L;");
    generate_test_file("C1.txt", "define C1(A,B)\n A -- B, 0R;1C;0L;");
    generate_test_file("L1.txt", "define L1(A,B)\n A -- B, 0R;0C;1L;");
    generate_test_file("R1L1.txt", "define R1L1(A,B)\n A -- B, 1R;0C;1L;");
    generate_test_file("R1C1.txt", "define R1C1(A,B)\n A -- B, 1R;1C;0L;");
    std::stringstream circuit_, answer_;
    ...
    switch (type) {
        ...
        case CircuitType::AC_RESISTANCE_CAPACITY:
            circuit_ << "1R; 1C; 0L; " << vertex_last << "V, 0;\ninclude R1C1.txt\n";
            answer_ << "0.707107 A, 45";
            break;
        case CircuitType::AC_RESISTANCE_INDUCTANCE:
            circuit_ << "1R; 0C; 1L; " << vertex_last << "V, 0;\ninclude R1L1.txt\n";
            answer_ << "0.707107 A, -45";
            break;
    }
...
TEST(FullyConnectedCircuitTest, FullyConnected100Test) { generate_fully_coherent_circuit_test(100/*, "FullyConnected100"*/); }
